...
const server = (0, http_1.createServer)((request, appResponse) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    if (request.method === 'POST') {
        ...
        if (testRequest.type === test_config_types_1.TestType.Graphql) {
            ...
            console.log(`[node] testPassed=${testPassed}, testFailedDebug=${testFailedDebug}\n`);
        }
        else {
            ...
            if (typeof testRequest.retryTimeoutTimer !== 'undefined') {
                ...
                console.log(`[node] RETRY_TIME_WAIT (BEFORE) = ${RestClient.RETRY_WAIT_TIME} ms\n\n`);
                if (testRequest.retryTimeoutTimer !== 0) {
                    console.log(`[node] setting setTimeout @ ${RestClient.RETRY_WAIT_TIME} ms\n`);
                    retryTimeout = setTimeout(() => {
                        try {
                            throw new Error('Request was not retried within the interval defined by Retry-After, test failed');
                        }
                        catch (error) {
                            console.log(`[node] ${RED}setTimeout fired!${RESET} @ ${RestClient.RETRY_WAIT_TIME}\n`);
